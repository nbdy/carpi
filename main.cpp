...
// todo module loader: loads all shared objects from directory, also provides a model for a repeater/column/whatever
// todo                should also allow the user to enable/disable modules on the fly and reload all modules / change the module directory
...
QJsonDocument parseArguments(QGuiApplication &app){
    ...
    QCommandLineOption optionGPS(QStringList() << "g" << "gps", QCoreApplication::translate("main", "Enable GPS"));
    QCommandLineOption optionWiFi(QStringList() << "w" << "wifi", QCoreApplication::translate("main", "Enable WiFi"));
    QCommandLineOption optionBluetooth(QStringList() << "b" << "bluetooth", QCoreApplication::translate("main", "Enable Bluetooth"));
    ...
    clip.addOptions({
        optionConfig, optionGPS, optionWiFi, optionBluetooth, optionMaps, optionStyle, optionStylesheetDirectory, optionCacheDirectory
    });
    ...
    if(clip.isSet(optionConfig)) cfg = loadConfig(clip.value(optionConfig));
        ...
        setValue(clip, optionGPS, "gps", o, clip.isSet(optionGPS));
        setValue(clip, optionWiFi, "wifi", o, clip.isSet(optionWiFi));
        setValue(clip, optionBluetooth, "bluetooth", o, clip.isSet(optionBluetooth));
