...
std::string FeatureTypeToStr(int typeIn) {
...
std::string ValueToStr(const ::vector_tile::Tile_Value &value) {
...
inline double CheckWindingi(LineLoop2Di pts) {
    ...
    for (size_t i = 0; i < pts.size(); i++) {
        ...
        double val = ((double) (pts[i2].first) - (double) (pts[i].first)) *
                     ((double) (pts[i2].second) + (double) (pts[i].second));
        total += val;
    }
    return -total;
}
...
void CheckCmdId(uint32_t cmdIdCount, int cmdId, size_t count) {
    ...
    if (cmdId != cmdId2 || count != cmdCount2)
        std::cout << "Failed to encode command (probably due to number of points)" << std::endl;
}
...
void DecodeVectorTile::DecodeTileData(const std::string &tileData) {
    ...
    if (!parsed)
        std::cout << "Failed to parse tile data" << std::endl;
...
void DecodeVectorTile::DecodeGeometry(const ::vector_tile::Tile_Feature &feature,
    ...
    std::vector<Point2D> points;
    std::vector<Point2Di> pointsTileSpace;
        ...
        if (cmdId == 2)//LineTo
        {
            for (unsigned j = 0; j < cmdCount; j++) {
                if (prevCmdId != 2) {
                    ...
                    pointsTileSpace.emplace_back(Point2D(prevxTileSpace, prevyTileSpace));
                }
                unsigned v = feature.geometry(i + 1);
                int value1 = ((v >> 1) ^ (-(v & 1)));
                v = feature.geometry(i + 2);
                ...
                pointsTileSpace.emplace_back(Point2D(cursorx, cursory));
                i += 2;
                prevCmdId = cmdId;
            }
        }
...
int lat2tiley(double lat, int z) {
    int y = (int) (floor(
            (1.0 - log(tan(lat * M_PI / 180.0) + 1.0 / cos(lat * M_PI / 180.0)) / M_PI) / 2.0 * pow(2.0, z)));
    int ymax = 1 << z;
    y = ymax - y - 1;
    return y;
}
...
void DecodeVectorTileResults::LayerStart(const char *name, int version, int extent) {
    ...
    std::cout << "layer version: " << version << std::endl;
}
...
void DecodeVectorTileResults::Feature(int typeEnum, bool hasId,
                                      unsigned long long id, const std::map<std::string, std::string> &tagMap,
                                      std::vector<Point2D> &points,
                                      std::vector<std::vector<Point2D> > &lines,
                                      std::vector<Polygon2D> &polygons) {
    std::cout << typeEnum << "," << FeatureTypeToStr((::vector_tile::Tile_GeomType) typeEnum);
    if (hasId)
        std::cout << ",id=" << id;
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < points.size(); i++)
        std::cout << "POINT(" << points[i].first << "," << points[i].second << ") ";
    for (size_t i = 0; i < lines.size(); i++) {
        std::cout << "LINESTRING(";
        std::vector<Point2D> &linePts = lines[i];
        for (size_t j = 0; j < linePts.size(); j++)
            std::cout << "(" << linePts[j].first << "," << linePts[j].second << ") ";
        std::cout << ") ";
    }
        ...
        if (polygon.second.size() > 0) {
            ...
            for (size_t k = 0; k < polygon.second.size(); k++) {
                ...
                for (size_t j = 0; j < linePts2.size(); j++)
                    std::cout << "(" << linePts2[j].first << "," << linePts2[j].second << ") ";
                std::cout << ") ";
            }
            std::cout << ")";
        }
...
EncodeVectorTile::EncodeVectorTile(int tileZoom, int tileColumn, int tileRow, std::ostream &output) : tileZoom(
        tileZoom), tileColumn(tileColumn), tileRow(tileRow), output(&output), currentLayer(NULL) {
    this->numTiles = pow(2, tileZoom);
    this->lonMin = tilex2long(tileColumn, tileZoom);
    this->latMax = tiley2lat(tileRow, tileZoom);
    this->lonMax = tilex2long(tileColumn + 1, tileZoom);
...
void EncodeVectorTile::LayerStart(const char *name, int version, int extent) {
    if (this->currentLayer != NULL)
        std::cout << "Previous layer not closed" << std::endl;
...
void EncodeVectorTile::LayerEnd() {
    if (this->currentLayer == NULL)
        std::cout << "Layer not started" << std::endl;
...
void EncodeVectorTile::Feature(int typeEnum, bool hasId, unsigned long long id,
                               ...
                               std::vector<Polygon2D> &polygons) {
    if (this->currentLayer == NULL)
        std::cout << "Cannot add feature: layer not started" << std::endl;
    ...
    for (std::map<std::string, std::string>::const_iterator it = tagMap.begin(); it != tagMap.end(); it++) {
        std::map<std::string, int>::iterator keyChk = this->keysCache.find(it->first);
        std::map<std::string, int>::iterator valueChk = this->valuesCache.find(it->second);
...
void EncodeVectorTile::EncodeTileSpacePoints(const std::vector<Point2Di> &points,
                                             int cmdId,
                                             bool reverseOrder,
                                             size_t startIndex,
...
void EncodeVectorTile::EncodeGeometry(vector_tile::Tile_GeomType type,
    ...
    if (outFeature == NULL) std::cout << "Unexpected null pointer" << std::endl;
    int cursorx = 0, cursory = 0;
    ...
    if (type == vector_tile::Tile_GeomType_LINESTRING) {
        for (size_t i = 0; i < lines.size(); i++) {
            const std::vector<Point2D> &line = lines[i];
